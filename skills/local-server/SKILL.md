...
### 호스트 이름에 따라 안 붙는 경우 (단, 단발 실패로 단정하지 말 것)
WSL2에서는 릴레이·바인딩 사정으로 `localhost` / `127.0.0.1` / `[::1]` 중 일부로만 붙는 일이
있다. 프론트가 특정 host를 하드코딩하면 **조용히 실패**한다(MOM_Generator의 "전문을 읽지
못했습니다" 원인).

**그러나 폴링 중 한 번 실패한 것은 근거가 못 된다.** 서버가 준비되는 순간에 걸리면
먼저 시도한 host가 실패하고 다음 host가 성공하는 일이 정상적으로 일어난다.
이 스킬도 처음엔 그걸 "바인딩 이슈"로 단정해 **거짓 경고**를 냈다(작업 규칙 2조 위반).

→ 판정은 **서비스가 뜬 것을 확인한 뒤** 문제의 host로 다시 쏴 본다. `health` 가 그렇게 한다.
→ 현재 이 WSL의 `/etc/hosts` 는 `localhost → 127.0.0.1` 하나뿐이고 `::1` 은 `ip6-localhost`
   라는 별도 이름이다. 즉 **`localhost` 와 `127.0.0.1` 은 같은 대상**이라 둘 사이에
   차이가 날 이유가 없다. 차이가 관측되면 타이밍부터 의심한다.

```bash
getent ahosts localhost     # 이 환경에서 localhost 가 무엇으로 풀리는지
ss -ltnp | grep <port>      # 실제 리스너가 IPv4/IPv6 어디에 붙었는지
```
